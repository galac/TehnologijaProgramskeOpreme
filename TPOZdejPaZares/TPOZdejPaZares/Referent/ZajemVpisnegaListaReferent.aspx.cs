...
{
    public partial class ZajemVpisnegaListaReferent : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (Session["studentekID"] == null)
                {
                    Server.Transfer("../UvodVVpisniList.aspx", true);
                }
                ...
                {
                    zetoniUporabnika = (from z in db.Zeton
                                        where z.Student_idStudent == uporabnikID
                                        && z.Vpis_idVpis == maxVpis.idVpis
                                        select z).ToList();
                }
        ...
        protected void buttonNadaljuj_Click(object sender, EventArgs e)
        {
            ...
            if (!Page.IsValid)
            {
                TextBoxNapake.Text += "Vse ni bilo validirano";
                return;
            }
            if (Regex.IsMatch(ime.Text, @"^[a-zA-ZčČšŠžŽćĆđĐ]+$") == false)
            {
                TextBoxNapake.Text += "Ime ni vredu";
                return;
            }
            if (Regex.IsMatch(priimek.Text, @"^[a-zA-ZčČšŠžŽćĆđĐ]+$") == false)
            {
                TextBoxNapake.Text += "Priimek ni vredu";
                return;
            }
            ...
            TextBoxNapake.Text += "Izvršeno. ";
            //Server.Transfer("IzbiraPredmetovReferent.aspx", true);
        }
