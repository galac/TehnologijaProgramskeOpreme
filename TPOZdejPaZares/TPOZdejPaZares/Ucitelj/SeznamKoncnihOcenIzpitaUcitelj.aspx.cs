...
{
    public partial class SeznamKoncnihOcenIzpitaUcitelj : System.Web.UI.Page
    {
        ...
        protected void GV_izpitni_roki_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (GV_izpitni_roki.SelectedIndex >= 0)
            {
                ...
                buttonPdf.Visible = false;
                buttonCsv.Visible = false;
                L_header51.Text = "Seznam rezultatov in končnih ocen pisnega dela izpita iz predmeta \"" + DDL_SubjectList.SelectedItem + "\"";
                ...
                //pridobimo seznam vseh študentov, ki so se udeležili te izvedbe predmeta
                var seznamVpisanihPredmetov = (from vp in db.VpisaniPredmet
                                               where vp.IzvedbaPredmeta_idIzvedbaPredmeta == ip.IzvedbaPredmeta_idIzvedbaPredmeta
                ...
                dt.Columns.AddRange(new DataColumn[8] { 
                                        ...
                                        new DataColumn("Ocena", typeof(int)),
                ...
                foreach (var prijava in seznamPrijav)
                {
                    ...
                    int koncnaOcena = 0;
                    foreach (VpisaniPredmet vp in seznamVpisanihPredmetov)
                    {
                        if (vp.Vpis.Student_idStudent1 == student.idStudent)
                        {
                            koncnaOcena = (vp.ocena != null) ? (int)vp.ocena : 0;
                        }
                    }
                    if (koncnaOcena == 0)
                    {
                    ...
                    dt.Rows.Add(zap, vpisnaStevilka, priimek, ime, stTock, koncnaOcena, studijskoLeto, zapStPolaganja);
                }
        ...
        protected void buttonExportToPdf_click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (anonimniIzpisCB.Checked == true)
                {
                    String[] headers = { "Št.", "Vpisna številka", "Št. točk", "Končna ocena", "Študijsko leto poslušanja", "Zap. št. polaganja" };
                    table.SetWidths(new int[] { 5, 15, 15, 15, 15, 15 });
                ...
                {
                    String[] headers = { "Št.", "Vpisna številka", "Priimek", "Ime", "Št. točk", "Končna ocena", "Študijsko leto poslušanja", "Zap. št. polaganja" };
                    table.SetWidths(new int[] { 7, 17, 15, 20, 13, 13, 15, 15 });
                    for (int i = 0; i < headers.Length; i++)
                    {
                        PdfPCell cell = new PdfPCell(new Phrase(headers[i], new Font(bfNormal, 12)));
                        cell.BackgroundColor = BaseColor.LIGHT_GRAY;
